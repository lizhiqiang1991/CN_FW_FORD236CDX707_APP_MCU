...
const uint8_t CRC_table_0x83[256] = 
...
uint8_t CRC8_Calculation(uint8_t *pu8data,uint8_t u8Length)
{
    ...
    uint8_t u8CrcTemp= NUM_ZERO; 
    uint8_t i = NUM_ZERO;  
    
    for(i=NUM_ZERO; i<u8Length; i++)
    {  
        // XOR datat byte into CRC 
        u8CrcTemp = (pu8data[i] ^ u8Crc); 
        // fetch CRC value from table 
        u8Crc = CRC_table_0x83[u8CrcTemp]; 
    }
